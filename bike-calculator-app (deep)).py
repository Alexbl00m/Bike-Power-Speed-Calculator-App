...
with col1:
    st.image("https://via.placeholder.com/150x150.png?text=Your+Logo", width=150)  # Replace with your logo URL
with col2:
    st.markdown("<div class='custom-header'>BIKE POWER SPEED CALCULATOR</div>", unsafe_allow_html=True)
...
# Define calculation functions outside tabs for reusability
...
with tab1:
    ...
    if target_type == "Power":
        ...
        _, f_rolling, f_grade, f_air = calculate_power(
            speed_ms, total_weight, avg_grade, cda, crr, 
            wind_speed_ms, air_density, drivetrain_efficiency
        )
    else:
        required_power, f_rolling, f_grade, f_air = calculate_power(
            target_speed_ms, total_weight, avg_grade, cda, 
            crr, wind_speed_ms, air_density, drivetrain_efficiency
        )
